...
{
	return 0;//mpage_readpage(page, aufs_get_block);
}

static int aufs_readpages(struct file *file, struct address_space *mapping,
			struct list_head *pages, unsigned nr_pages)
{
	return 0;//mpage_readpages(mapping, pages, nr_pages, aufs_get_block);
}
			...
			struct iov_iter *iter, loff_t off)
{
	...
	return 0;//blockdev_direct_IO(rw, iocb, inode, iter, off, aufs_get_block);
}
